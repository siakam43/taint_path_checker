...
- **调用点记录**：追踪污点在函数间传递时，记录上一函数调用下一函数的调用点行号（call site line）。此信息用于生成最终报告的攻击路径章节。
...
[2]   :49                    func_dispatch()
         :230 ──→ func_process()
[3] /绝对路径/file_b.c:230   func_process()
         :78 ← 触发点
```

格式规则：
- `[N]` 步骤序号。
- 函数定义位置：`文件:行号  函数名()`。同文件连续时后续步骤只写 `:行号`；跨文件时补全 `/绝对路径/file.c:行号`。
- 箭头前 `:行号` 为调用点（call site），即上一步函数在何处调用了下一步函数。
- 最后一步末尾用 `← 触发点` 标注漏洞触发位置。
