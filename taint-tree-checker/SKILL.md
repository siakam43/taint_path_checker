...
注意区分函数调用关系和污点传播数据流的区别：调用树中的一条边（如 a→b）只表示"存在调用关系"，**不代表"只调用一次"**。调用方可能多次调用同一函数，各调用点的实参不同、校验情况也不同。每种情况都是一个独立的污点传播数据流路线。
...
- **逐污点传播数据流路线分析**：通过不同调用点到达同一使用点时，每个"调用点→使用点"组合都是独立的污点传播数据流路线。
...
对 5.2 定位到的每个危险使用点，逐一检查其每条污点传播数据流路线（见 5.1）的实参在到达使用点前是否存在任何形式的校验。
...
**每条污点传播数据流路线中的校验独立分析，即使两条数据流中的函数调用一模一样，但由于走的条件分支不同、函数调用点不同、污点数据变量，导致校验分析结果也不同。**

候选漏洞的攻击路径必须标注每个具体调用点的行号。
