...
- rel 一旦确定，全任务禁止再出现绝对路径形态的文件引用：报告（7.4 所在文件、攻击路径、代码片段注释）与漏洞哈希（7.1）一律使用 rel；绝对路径仅作为读取源码时的中间拼接值，不得写入任何产物。
...
1. 以 4.1 标记的初始污点为起点，以调用树信息为参考，按照 4.2 污点传播模式、结合 4.4 追踪注意事项进行污点追踪，完整追踪污点数据的不同传播路线，记录每条污点传播路线中的具体函数调用点及参数、执行的条件分支、污点在变量间的传递过程等信息。整个阶段结束前，强制执行一次全局的"4.3.3 完整性自检"。最终产出完整且准确的污点传播数据流路线集合，覆盖污点可达的全部代码。
2. 在追踪污点传播数据流路线过程中，遇到调用深度过大或单个调用分发点目标过多时，按照"4.3.2 追踪上限"规则进行规模收敛。
3. 在追踪污点传播数据流路线过程中，如果调用树信息不完整，按照"4.3.1 调用树修复规则"对调用树信息进行修正，修复结果（含扇出解析出的目标回调）按落盘规则写入修复树文件。
...
- **落盘**：有修复时，将修复后的调用树写入 `{project_dir}/.ethunter_out/taint-tree-checker/{TreeID}_tree_fixed.json`，格式与输入 JSON 一致（file 字段使用规范化相对路径）；未修复则不生成该文件。不得修改输入文件。**扇出解析出的目标回调（上限内）与树外延伸链均属于修复内容，必须补入修复树落盘；超出上限的剩余回调不补入，仅在摘要逐条列出。**

**4.3.2 追踪上限（目标可确定但规模巨大时）：**
...
- **上限内必须完整追踪到终态**：上限之内的每个回调/分支，均须追踪至 4.3.3 完整性自检的终态之一（停止传播/无害消费/净化/危险使用点），不得中途以"复杂"为由跳过。
...
**4.3.3 完整性自检（本阶段结束前强制执行）**：每个污点变量必须追踪到以下状态之一——停止传播（不再传递）、被无害消费（使用但无危险模式）、被净化（校验后）、到达危险使用点、标注"无法追踪"（**仅限两类**：间接调用目标不可确定且已写明搜索方式；源码缺失。**"数量众多/链路深"不属于无法追踪**，规模超上限的按"追踪上限"规则在摘要逐条列出）。存在未达上述任一状态的污点变量时，不得进入第五章的危险使用扫描。

### 4.4 追踪注意事项

- **同名变量区分**：分析代码上下文，结合作用域和数据流，区分同名不同作用域的变量。
- **结构体成员区分**：结构体整体为污点则全部成员为污点；若污点数据赋值给某成员，则仅该成员为污点。
- **树外函数追踪**：污点传递到树外函数时，查找该函数定义，分析其对污点的操作（污染了哪些新变量、是否有校验）。污点在该函数内继续传播到新的树外函数时，按 4.3.1 调用链缺失/中断规则修复调用树；污点在该函数内停止（被消费或净化）时不补入树。
...
以第四步的污点传播数据流路线为分析对象，逐条数据流路线分析是否存在因外部污点数据未正确校验导致的安全漏洞。**污点传播数据流不仅包含函数调用关系，还包含具体的函数调用点及对应的参数，执行的条件分支，污点数据在变量间的传播关系等信息。**

注意区分函数调用关系与污点传播数据流：调用树中的一条边（如 a→b）只表示"存在调用关系"，**不代表"只调用一次"**。调用方可能多次调用同一函数，各调用点的实参不同、校验情况也不同。每种情况都是一个独立的污点传播数据流路线。
...
- **逐污点传播数据流路线分析**：经不同调用点或不同条件分支到达同一使用点时，每个"调用点→使用点"组合都是独立的污点传播数据流路线。
...
对 5.2 定位到的每个危险使用点，逐一检查其每条污点传播数据流路线（见 5.1）的污点数据在到达使用点前是否存在任何形式的校验。
...
对每条"存在危险使用点"的污点传播数据流路线：无校验，或校验不完备（按 5.4 判定，须给出具体绕过路径）→ 记为候选漏洞。

**每条污点传播数据流路线的校验独立判定：一条路线已正确校验，不代表另一条路线已正确校验——即使两条路线的函数调用一模一样，走的条件分支、函数调用点或污点数据变量不同，校验结论也可能不同。任一路线未正确校验即构成触发路径。**

候选漏洞的攻击路径必须**落在未正确校验的污点传播数据流路线上**，并标注该路线中每个调用点的行号。
...
其中 `hash8` = 对字符串 `{rel}:{函数名}:{起始行号}` 计算 SHA256，取前 8 位十六进制字符。rel 为按 2.6 转换得到的规范化相对路径。**哈希输入必须使用 rel 本身，不得使用 `{project_dir}/{rel}` 拼接结果或任何绝对路径形态——即使定位源码时做过拼接。** **起始行号为漏洞关键行号的起始行**（与 7.4 模板的关键行号字段一致），不是函数定义行。
...
6. 漏洞 ID 是否基于规范化相对路径的 file:func:line 正确生成，树内同 ID 漏洞是否已合并。
7. 是否有排除清单中的问题类型被误报。
...
| "调用树中的每条边分析一次就够了" | 同一条边可对应多个调用点。同函数多次调用、实参不同时，是不同的污点传播数据流路线，需逐条路线进行分析，任一路线未正确校验即构成漏洞 |
| "这个漏洞在两条链里都发现了，各报一次" | 树内同 ID 漏洞（同 file:func:line）合并报告一次，攻击路径须选取确实可达的链 |
| "输入 file 是相对路径，先转成绝对路径再用" | 哈希与报告中的路径必须使用规范化相对路径 rel；绝对路径仅限读取源码时的 `{project_dir}/{rel}` 拼接，不得出现在报告或哈希输入中 |
