...
3. 若某链中某函数源码在给定文件中不存在，**跳过该链**，继续分析其余链，在最终报告摘要中注明被跳过的链及原因。不得因个别链缺失而中止整棵树的分析。
...
对每条链独立执行本节的追踪流程。**全局/静态变量污点在链间不传播**：每条链从干净的全局污点状态开始分析，链内某函数写入的全局/静态变量污点仅对同链内后续函数生效。调用树描述的是同一入口出发的备选执行路径，一次调用只沿其中一条链执行——不得以"链1写入的全局变量在链2中残留"为由跨链传播污点，否则会产生实际不可触发的误报。
...
追踪污点传播时在链外辅助函数中发现的漏洞，确认非误报后，在报告中标注 **"是否链外：是"**，并在漏洞描述中说明该函数与当前链的调用关系。
...
| "两条链是同一程序的备选路径，链1写入的全局变量链2执行时会残留" | 一次调用只沿一条链执行。跨链传播污点会产生实际不可触发的误报。链间独立：每条链从干净的全局污点状态开始分析 |
...
| "键中 `$` 后缀只是命名元数据，不影响分析，可以忽略" | TreeID 是预查重与输出命名的依据，必须以文件名为准并校验；不一致时报错结束，不得忽略校验继续分析 |
