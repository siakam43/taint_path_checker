...
new:
3. 若某函数源码在给定文件中不存在：该函数**可能接收污点**（实参含污点或无法确认实参是否污点）→ 该链污点状态未知，跳过该链、其余链照常分析，摘要注明"函数 xxx 源码未找到，跳过该链"；**可确认不接触污点**（实参均非污点）→ 继续该链分析，摘要注明"函数 xxx 源码未找到，其不接触污点数据，分析继续"。仅按调用点实参判定：无函数源码时无法判断其是否读取已污染的全局变量，故全局读取不作为判定因素。不得因个别函数缺失而中止整棵树的分析。
...
**完整性自检（本阶段结束前强制执行）**：每个污点变量必须追踪到以下状态之一——停止传播（不再传递）、被无害消费（使用但无危险模式）、被净化（校验后）、到达危险使用点、标注"无法追踪"（间接调用目标不明或源码缺失，记录于摘要）。存在未达上述任一状态的污点变量时，不得进入第五章的危险使用扫描。
```
...
通读 `taint-tree-checker/SKILL.md` 全文，对照 spec 逐章修改设计核对：二.3 落盘说明、三.3 源码缺失规则、4.1 全树共享、4.2(3) 树级措辞、4.3 修复规则+五态自检、4.4 联动、六.1 跨调用前提、7.3/7.4 覆盖结论与修复情况、7.5 标题、八.2/八.3、9.1 跨链陷阱、9.2 三行。任一缺失则补上。
