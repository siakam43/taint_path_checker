...
new:
3. 若某函数源码在给定文件中不存在：该函数**可能接收污点**（参数为污点或读取已污染的全局/静态变量；无法确认时按可能接收处理）→ 该链污点状态未知，跳过该链、其余链照常分析，摘要注明"函数 xxx 源码未找到，跳过该链"；**可确认不接触污点**（实参均非污点、不读取污点全局变量）→ 继续该链分析，摘要注明"函数 xxx 源码未找到，其不接触污点数据，分析继续"。不得因个别函数缺失而中止整棵树的分析。
...
new:
| 调用树中某函数源码找不到 | 可能接收污点（无法确认按可能接收处理）→ 跳过该链，其余链照常，摘要注明"函数 xxx 源码未找到，跳过该链"；可确认不接触污点（实参均非污点、不读取污点全局变量）→ 继续该链，摘要注明"函数 xxx 源码未找到，其不接触污点数据，分析继续" |
