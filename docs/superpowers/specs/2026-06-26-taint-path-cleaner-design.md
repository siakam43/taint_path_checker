...
- 若类型属于 3.1 B组 → 进入第四章可触发性深度复核。
- 若类型不属于 3.1 B组 → 直接判定为误报，记录至 FPs/FPs.md。
...
1. 定位攻击路径中每一步的源代码文件。若文件在 project_dir 中不存在，注明无法验证。
...
| 类型窄化 | `uint8_t v = tainted_uint32;` | 截断至 0~255，作为缓解因素 |
...
3. 若安全性无法确认（例如：污点数据虽未校验，但经过多层间接传递后取值已经被隐式约束），按误报处理。
...
- **判定结果**：误报
- **误报原因**：（简要说明误报原因，如"类型不在关注范围：内存泄漏"、"存在隐式校验：switch-case 限制了 idx 取值范围"、"触发路径不可达：入口函数为内核内部接口，无外部输入"）
```

每次追加新记录时，在文件开头（若文件已存在则在原有内容之后）追加。若同一 TAINT-xxxxxxxx 在文件中已存在，需避免重复。若虽然 ID 相同但问题不同（极少情况），将新问题追加写入同一文件。
...
1. 输入报告中的每个漏洞是否都有对应的输出（Vulns/ 中的正报文件 或 FPs/FPs.md 中的误报记录，或已在预查重阶段跳过）。
2. 每个判定是否有明确的代码证据或逻辑依据。
3. 误报记录是否包含 TAINT-xxxxxxxx、判定结果、简要原因三项。
