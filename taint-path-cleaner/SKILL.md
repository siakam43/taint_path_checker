...
5. 若攻击路径中调用关系不成立，应先尝试自行补全路径（搜索函数引用、追踪间接调用）。仅当补全后调用关系仍不成立，或入口函数确认不可被外部触发，该漏洞按误报处理。

6. **链外漏洞的额外处理**：若 checker 报告中标注"是否链外：是"，需额外验证该链外函数是否确实能被调用链中的函数触发（确认链内→链外的调用关系）。若调用关系成立，其余分析与链内漏洞一致；若调用关系无法确认，按误报处理。
