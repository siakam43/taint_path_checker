...
1. 检查 `{project_dir}/.ethunter_out/taint-path-cleaner/Vulns/TAINT-xxxxxxxx.md` 是否存在 —— 若存在，说明此前已判定为正报，跳过该漏洞。
2. 检查 `{project_dir}/.ethunter_out/taint-path-cleaner/FPs/FPs.md` 中是否已记录该 TAINT-xxxxxxxx —— 若已记录，说明此前已判定为误报，跳过该漏洞。
...
输出路径：`{project_dir}/.ethunter_out/taint-path-cleaner/Vulns/TAINT-xxxxxxxx.md`
...
- 每次追加新记录时，在文件末尾追加。
- 若需多次写入同一文件，不要覆盖已有内容。
- 若同一 TAINT-xxxxxxxx 在文件中已存在，需避免重复写入。
- 若虽然漏洞 ID 相同但问题不同（极少情况），将新问题追加写入，并注明与前次记录的区别。
...
3. 误报记录是否包含 TAINT-xxxxxxxx、判定结果、误报原因三项。
...
| FPs/FPs.md 中已存在同一 ID 但漏洞描述不同 | 追加写入，注明与新问题不同 |
| Vulns/TAINT-xxx.md 已存在 | 跳过，不重复分析 |
...
- **Read**：读取 checker 报告、project_dir 中的源代码文件
- **Bash**：创建输出目录（mkdir -p）、在 project_dir 中搜索函数定义和符号（grep、find）
